...
if (strcmp($auth->getAuth(), 'admin') != 0) {
	...
	if (!isset($_POST['ip'])||!isset($_POST['ban_time'])||!isset($_POST['remarks']))
	{
		...
		Ip_block::update($_POST['ip'], $_POST['ban_time'], $_POST['remarks']);
		$success = true;
		$errorType = 'success';
		$ajaxRedirect = '?controller=admin&action=ip_block';
		User_log::insert($_SESSION["uid"], "Admin", "Updated IP block: ".$_POST['ip'].",".$_POST['ban_time'].",".$_POST['remarks'], 3, implode(",",$old));
	}
